...
# Limit concurrent DB processing to prevent CPU saturation waves (thread-safe for multi-event-loop execution)
DB_PROCESS_SEMAPHORE_LIMIT = 3
db_process_semaphore = threading.Semaphore(DB_PROCESS_SEMAPHORE_LIMIT)
...
async def run_room_poll(room_info, loop):
    ...
    try:
        def _throttled_db_process():
            with db_process_semaphore:
                return db_process_poll_data(
                    db_id,
                    room_uuid,
                    tracker_data,
                    room_data,
        ...
        notifications_to_send = await loop.run_in_executor(None, _throttled_db_process)
...
async def run_room_setup(room_info, loop):
    ...
    try:
        ...
        if port:
                    ...
                    try:
                        ...
                        async with session.ws_connect(uri, timeout=5, max_msg_size=16*1024*1024) as ws:
                            ...
                            if msg.type == aiohttp.WSMsgType.TEXT:
                                ...
                                if checksums:
                                    missing_checksums = await loop.run_in_executor(None, db_get_missing_checksums, list(checksums.values()))
            ...
            if not ws_success:
                 logging.warning(f"[POLLER_SETUP_WARN][RoomDBID:{db_id}] WebSocket setup failed. Cannot fetch checksums.")
                 any_datapackage_failed = True
...
async def poll_room_with_interval(room_info, loop):
    ...
    while True:
        ...
        try:
            ...
            sleep_duration = max(1.0, POLLING_INTERVAL_SECONDS + jitter)
